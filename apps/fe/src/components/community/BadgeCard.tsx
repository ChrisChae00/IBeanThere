  ...
  return (
    ...
    >
      ...
      <div className={`relative w-24 h-24 mb-4 rounded-full flex items-center justify-center
        ${isUnlocked ? 'bg-primary/10 ring-4 ring-primary/5' : 'bg-border/30'}`}
      >
        {isUnlocked ? (
          badge.icon_url ? (
            <Image
        ...
        {isUnlocked && (
          <div className="absolute inset-0 rounded-full bg-linear-to-tr from-white/0 via-white/20 to-white/0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" />
        )}
      </div>
      ...
      {!isUnlocked && (
        <div className="absolute inset-0 z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none">
          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 p-3 bg-black/90 backdrop-blur-xs text-white text-xs rounded-lg shadow-xl text-center">
            <p className="font-semibold mb-1 text-primary">{t('how_to_unlock')}</p>
