      ...
      <div className="mb-6">
        ...
        <div className="flex gap-3 overflow-x-auto pb-4 px-4 scrollbar-hide">
          {[1, 2, 3, 4, 5].map((i) => (
            <div
              key={i}
              className="shrink-0 w-16 h-16 rounded-full bg-muted animate-pulse"
            />
          ))}
        </div>
      </div>
      ...
      <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
        {tasteMates.map((mate) => (
          <Link
            ...
            className="shrink-0 flex flex-col items-center gap-2 group w-16"
          >
            <div className="relative">
              <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-border group-hover:border-primary transition-colors bg-surface">
                {mate.avatar_url ? (
                  <Image
                    src={mate.avatar_url}
            ...
            <span className="text-xs text-text truncate w-full text-center">
              {mate.display_name}
            </span>
          </Link>
        ))}
