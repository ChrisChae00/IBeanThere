    ...
    <div className="relative w-full" style={{ paddingBottom: '100px' }} ref={containerRef}>
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-5 pt-1 px-4 md:px-8 lg:px-10">
          ...
          return (
            <div key={stage} className="flex justify-start pt-10 md:pt-36 md:gap-10">
              ...
              <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
                <div className="h-12 absolute left-2 md:left-3 w-12 rounded-full bg-background border-2 border-border flex items-center justify-center shadow-lg shrink-0 overflow-hidden">
                  {STAGE_ICONS[stage]}
                </div>
                <h3 className="hidden md:block text-xl md:pl-20 md:text-3xl font-bold text-text opacity-85">
                  {data.title}
                </h3>
              </div>
              ...
              <div className="relative pl-20 pr-4 md:pl-4 w-full">
                ...
                <motion.div
                  ...
                  className="rounded-2xl border border-border bg-surface p-6 md:p-8 hover:shadow-lg transition-shadow duration-300"
                >
                  <div className="mb-4">
                  ...
                  <p className="text-text font-medium text-base md:text-lg mb-6 leading-relaxed">
                    {data.description}
                  </p>
                  <div className="border-t border-border pt-4 flex flex-wrap gap-2 justify-center">
                    {data.highlights.map((highlight, i) => (
                      <span
                        key={i}
                        className="text-xs md:text-sm bg-primary/5 text-text px-3 py-1.5 rounded-lg"
                      >
                        {highlight}
                      </span>
                    ))}
        ...
        <div
          ...
          className="absolute md:left-8 left-8 top-0 overflow-hidden w-[2px] bg-linear-to-b from-transparent via-border to-transparent mask-[linear-gradient(to_bottom,transparent_0%,black_10%,black_90%,transparent_100%)]"
        >
          <motion.div
            style={{ height: heightTransform, opacity: opacityTransform }}
            className="absolute inset-x-0 top-0 w-[2px] bg-linear-to-t from-primary via-primary/80 to-transparent rounded-full"
          />
        </div>
      </div>
    </div>
