    ...
    <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 relative z-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 lg:items-center">
        ...
        <div className="flex flex-col gap-6 w-full">
          <div>
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-primaryText mb-4 break-keep leading-tight">
              {messages.title}
            </h2>
            <p className="text-base sm:text-lg text-background break-keep leading-relaxed">
              {messages.subtitle}
            </p>
          </div>
          ...
          <Accordion.Root
            ...
            defaultValue=""
            collapsible
            className="w-full rounded-xl overflow-hidden border border-primaryText/20"
          >
            {accordionData.map((item, idx) => (
              <Accordion.Item
                key={item.value}
                value={item.value}
                className={idx < accordionData.length - 1 ? 'border-b border-primaryText/20' : ''}
              >
                <Accordion.Header>
                  <Accordion.Trigger className="group flex w-full items-center justify-between px-5 py-4 text-left text-sm sm:text-base font-semibold text-primaryText hover:bg-primaryText/5 transition-colors duration-200 [&[data-state=open]>svg]:rotate-180">
                    {item.title}
                    <ChevronDown
                      className="h-4 w-4 shrink-0 text-primaryText/60 transition-transform duration-300 ml-3"
                      aria-hidden="true"
                    />
                  </Accordion.Trigger>
                </Accordion.Header>
                <Accordion.Content className="overflow-hidden data-[state=open]:animate-[accordionDown_200ms_ease-out] data-[state=closed]:animate-[accordionUp_200ms_ease-out]">
                  <p className="px-5 pb-4 pt-1 text-sm sm:text-base text-background leading-relaxed text-left">
                    {item.desc}
                  </p>
                </Accordion.Content>
              </Accordion.Item>
          ...
          <a
            ...
            className="inline-flex items-center justify-center w-full sm:w-auto px-8 py-4 text-base sm:text-lg font-semibold rounded-full bg-background text-text hover:bg-primaryText transition-all hover:scale-105 shadow-[0_15px_40px_rgba(0,0,0,0.25)] min-h-[56px]"
          >
            {messages.cta}
          </a>
        </div>
