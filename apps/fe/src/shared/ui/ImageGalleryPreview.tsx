    ...
    <div className={`${className}`}>
      <div className={`grid ${getGridClasses()} gap-2 rounded-xl overflow-hidden`}>
        {displayImages.map((image, index) => {
          ...
          return (
            <div
              key={index}
              className={`relative aspect-square cursor-pointer overflow-hidden bg-surface ${getItemClasses(index, displayImages.length)}`}
      ...
      {images.length > 1 && onViewAllClick && (
        <button
          ...
          className="mt-3 w-full py-2.5 px-4 text-sm font-medium text-text bg-surface hover:bg-(--color-surface-hover) border border-border rounded-lg transition-colors flex items-center justify-center gap-2"
        >
