...
카페 태그 무효화는 원래 점검 대상에 이미 포함된 구현이다. 따라서 그 커밋만으로는
기존 발견 사항을 해결 상태로 바꾸지 않았다. 커밋 전 백엔드 59개 테스트와
프론트엔드 프로덕션 빌드 통과 기록은 기능 검증이며, 보안 재검증이나 운영 설정 확인을
대신하지 않는다. 이번 문서 갱신에서는 외부 취약점 조회와 운영 검증을 다시 수행하지 않았다.

수정(2026-09-09, 문서 동기화 이후): **SEC-07은 해결됐다** — 로그 경로의 자동 승인이
서버가 계산한 위치 증거를 요구하고, 테스트가 이를 고정한다. **SEC-10은 부분 해결됐다** —
캐시 태그 불일치, 대표 이미지 복사 경로, Server Action 방어는 처리했고, 백엔드에서의
캐시 무효화는 남아 있다. 나머지 발견 사항은 그대로 열려 있다.

수정(2026-09-11): **SEC-11의 신고 메일 HTML 주입은 해결됐다**(`cf74226`). 좋아요 경로가
남아 있어 SEC-11 전체는 부분 해결이다. 인증 화면 개편과 커밋 보안 리뷰 과정에서 새로 찾은
세 건을 SEC-14~16으로 추가했다 — 카페 상세의 저장형 XSS와 신고 첨부 URL 무검증은 해결,
비밀번호 재설정의 사용자 열거는 폼 경로만 해결이다. 검증은 백엔드 82개 테스트(신규 10개),
프론트엔드 `tsc`·검사 스크립트 2개·프로덕션 빌드다. 운영 DB에 이미 저장된 신고·카페 행은
조회하지 않았다.
...
로그 경로의 자동 승인 조건을 서버에서 명시적으로 검사한다. `mode == "purchase"`이고
**엔드포인트가 카페 좌표로 직접 계산한** 거리가 있을 때만 `approved`다. 그 거리는 50m
게이트를 통과한 체크인에서만 생기므로, 등록의 100m 게이트와 같은 성격의 증거다. 요청
본문의 `distance_meters`는 클라이언트가 적은 숫자라 근거로 쓰지 않는다. 나머지 로그는
카페 상세 제안과 동일하게 `pending`으로 저장돼 관리자 큐로 간다.

현재 로그 폼은 좌표를 보내지 않는다. 따라서 실제로는 모든 로그의 `sells_beans`가 검토
대기이며, 즉시 반영은 등록 경로에만 남는다. 로그 폼에 위치를 붙이는 것은 별도 작업이다 —
지금 붙이면 집에서 쓴 로그가 `DISTANCE_TOO_FAR`로 거부되므로, 엔드포인트가 먼 좌표를
거부 대신 "증거 없음"으로 다루도록 함께 바꿔야 한다.

`tests/test_trait_suggestions.py`의 `LogPathEvidenceTests`가 네 경우(drink 로그, 좌표
없는 구매, 클라이언트가 적은 `distance_meters`, 게이트 안 체크인)를 고정한다. 백엔드
테스트 65개 통과.
...
- 심각도: **Medium**
- 상태: **부분 해결** (2026-09-09) — 권고 1·3·4 반영, 2는 미해결. 아래 "조치" 참조
...
1. **캐시 태그 불일치 해결.** `revalidateCafe`가 `cafe-{식별자}`와 함께 상세 fetch가 항상
   달고 있는 넓은 `cafe` 태그를 무효화합니다. 상세 페이지는 URL이 실어온 식별자(대개 slug)로
   캐시되는데 호출자는 전부 UUID를 들고 있어, 좁은 태그만으로는 slug 캐시가 2분간 살아
   있었습니다. 다른 카페 페이지까지 같이 버리지만 현재 규모에서는 캐시 미스 몇 건입니다.
3. **대표 이미지 복사 경로 제거.** `POST /admin/backfill-cafe-images`를 삭제했습니다.
   공개 로그 사진을 `cafes.main_image`로 복사하면서 출처 로그를 남기지 않아, 그 로그가
   비공개로 바뀌거나 삭제돼도 복사본이 계속 노출됐습니다. 출처 컬럼을 추가하는 대신
   삭제한 이유: 이 엔드포인트가 피하려던 런타임 조회가 `cafes.py`에 이미 있고(트렌딩·상세
   양쪽), 그쪽은 매 요청 `is_public`을 다시 읽어 철회가 즉시 반영됩니다. 이제
   `main_image`는 등록·관리자 편집·검수된 시드처럼 **의도한 행위로만** 기록됩니다.
   기존 데이터도 확인했습니다: 저장된 `main_image` 12건 중 **비공개 로그의 사진을 가리키는
   것은 없습니다**(공개 로그에 있는 것 8건, 어느 로그에도 없는 것 4건 = 등록·관리자·시드).
   즉 지금 노출되고 있는 철회된 사진은 없습니다.
4. **Server Action 방어.** 세션(`supabase.auth.getUser()`)과 식별자 형식을 검사하고,
   아니면 조용히 아무것도 하지 않습니다. 캐시 힌트에는 오류를 보고할 상대가 없습니다.

미해결 (권고 2): 백엔드에서의 무효화. FastAPI가 Next 캐시에 접근할 수단이 없어(시크릿을
쓰는 revalidate 엔드포인트가 필요), 이 프론트엔드를 거치지 않은 공개 범위 변경은 여전히
아무것도 무효화하지 않습니다.
...
- 심각도: **Medium**
- 상태: **부분 해결** — 신고 메일 주입은 `cf74226`에서 해결, 비공개 로그 좋아요는 열려 있음
...
조치(2026-09-11, `cf74226`): 권고 3·4를 반영했다. 신고 알림·처리 결과 메일의 모든 보간값을
`html.escape`로 처리하고, 링크는 http(s)일 때만 만든다(`href`에서는 escape가 `javascript:`를
막지 못한다). 입력 단계에서도 `target_url`을 http(s)로 제한한다. 호스트 범위는 제한하지
않았다 — 신고 대상은 앱 자신의 페이지지만, 배포 도메인 목록이 백엔드 설정과 어긋나면 정상
신고가 거부되기 때문이다. 좋아요 경로(권고 1·2)는 그대로 열려 있다.
...
### SEC-14. 카페 상세 구조화 데이터의 저장형 XSS

- 심각도: **High**
- 상태: **해결** (`cf74226`, 2026-09-11 발견)
- 관련 코드: `apps/fe/src/app/[locale]/cafes/[id]/page.tsx`

카페 상세 페이지는 이름·주소·전화·웹사이트를 `JSON.stringify`로 직렬화해
`<script type="application/ld+json">`에 그대로 넣었다. `JSON.stringify`는 `</script>`를
escape 하지 않으므로, 카페 이름이 `</script><script>…`이면 태그가 닫히고 뒤의 스크립트가
그 카페 상세를 여는 모든 방문자의 브라우저에서 실행된다. 카페 데이터는 사용자 등록과 외부
지도 데이터에서 온다.

재현은 직렬화 결과만 확인했다(운영 DB라 악성 카페 행을 만들지 않았다).

조치: 직렬화 결과의 `<`를 `\u003c`로 치환한다. JSON 파서에는 같은 문자이고 HTML 파서에는
태그가 아니다. 학습 페이지와 랜딩의 JSON-LD는 코드 안의 정적 데이터라 해당하지 않는다.

남은 확인: 운영 DB에 `<`를 포함한 카페 이름·주소가 이미 있는지.

### SEC-15. 비밀번호 재설정의 사용자 열거

- 심각도: **Low**
- 상태: **부분 해결** (`ed6d4f6`) — 폼 경로 해결, API 직접 호출과 회원가입은 **조건부**
- 관련 코드: `apps/fe/src/features/auth/presentation/components/ForgotPasswordForm.tsx`

Supabase `/recover`는 가입하지 않은 주소에는 발송 없이 성공을 반환한다. 따라서 이 호출이
돌려주는 에러(계정별 재발송 대기, 기본 메일러의 수신자 거부, 발송 실패, 이메일 발송 한도)는
가입된 주소에서만 발생한다. `cd3f579`는 발송 실패를 숨기던 버그를 고치며 에러를 그대로
보여줬고, 그 결과 에러 여부로 가입 여부를 판별할 수 있었다. 커밋 보안 리뷰가 발견했다. 원래
코드의 `includes('rate')`도 계정별인 "email rate limit exceeded"를 보여주던 작은 누출이었다.

조치: 프로덕션에서는 계정 조회 전에 걸리는 IP 요청 제한만 표시하고, 그 외 에러는 발송 완료
화면으로 보낸다. 완료 문구를 "계정이 있다면 보냈다"로 바꿔 모든 주소에 참이 되게 했다. 개발
환경에서만 실제 에러를 표시한다.

남은 것:

1. 공개 anon key로 `/recover`를 직접 호출하면 같은 신호가 보인다. Supabase 동작이라 앱에서
   막을 수 없고, CAPTCHA와 rate limit으로 비용을 높이는 것이 대응이다(SEC-08).
2. 회원가입 폼은 Supabase의 "User already registered"를 번역해 보여준다. Confirm email이
   꺼져 있으면 기가입 주소가 드러난다. 켜면 Supabase가 가짜 성공을 반환하지만, 현재 가입
   흐름이 즉시 세션을 가정하므로 인증 대기 화면을 먼저 만들어야 한다. 설정값은 확인하지 않았다.

### SEC-16. 신고 대상 URL과 첨부 URL 무검증

- 심각도: **Medium**
- 상태: **해결** (`cf74226`, 2026-09-11 발견) — 기존 행은 미확인
- 관련 코드: `apps/be/app/models/report.py`, `apps/be/app/api/v1/reports.py`, `apps/fe/src/features/admin/data/AdminReportsRepository.ts`

`target_url`은 스킴 제한 없이 저장되어 관리자 메일과 대시보드의 링크가 됐다. `image_urls`는
임의 문자열 3개까지 저장되어 관리자 대시보드가 `<img>`로 렌더했다. 신고자는 외부 호스트
이미지로 관리자가 신고를 연 시점과 IP를 수집하거나(추적 픽셀), 다른 사용자의 업로드를
첨부로 가리킬 수 있었다. React 19는 `javascript:` href를 막지만 메일 클라이언트는 그렇지 않다.

조치:

1. 입력에서 `target_url`은 http(s)만, 2048자 이하로 받는다.
2. 첨부는 `reports` 버킷의 신고자 본인 폴더에 있는 단일 경로 세그먼트 파일만 받는다.
   `/`, `\`, `%`, `..`는 폴더 밖으로 경로를 옮길 수 있어 거부한다.
3. 대시보드가 신고를 읽는 매핑 한 곳에서 같은 두 필드를 거른다(검증 도입 전 행 대비).
4. `apps/be/tests/test_report_safety.py`가 URL 검증, 첨부 소유권, 메일 escape를 고정한다.

첨부 버킷의 공개 여부(SEC-05)는 이 조치와 별개로 열려 있다.
...
| 신고 메일 HTML (2026-09-11 재검증) | 가짜 sender에서 `<script>`·속성 탈출 escape, `javascript:` 링크 미출력 확인 (`cf74226`) |
| 신고 URL·첨부 검증 (2026-09-11) | `javascript:`·`data:`·상대 URL 422, 타인·외부·경로 탈출 첨부 거부 테스트 통과 |
| 카페 JSON-LD 직렬화 (2026-09-11) | `</script>` 포함 이름이 `\u003c`로 직렬화되고 JSON 파싱 시 원문 복원 확인 |
...
6. 회원가입에 인증 대기 화면을 만든 뒤 Confirm email을 켜서 기가입 주소 노출을 닫는다(SEC-15).
...
- [x] 사용자 제공 신고 문구와 URL이 메일 HTML에서 escape 된다. (`cf74226`, 테스트)
- [x] 신고 `target_url`은 http(s)만, 첨부는 신고자 본인의 `reports` 폴더 파일만 저장된다. (`cf74226`, 테스트)
- [x] 카페 이름에 `</script>`가 있어도 상세 페이지 구조화 데이터 밖으로 나오지 않는다. (`cf74226`, 직렬화 확인)
- [ ] 프로덕션 비밀번호 재설정 폼이 가입·미가입 주소에 같은 화면을 보인다. (`ed6d4f6`, 운영 미확인)
- [ ] 회원가입이 기가입 주소를 드러내지 않는다. (Confirm email 설정 필요)
- [ ] 운영 DB의 기존 신고 행에 http(s) 외 `target_url`이나 외부 첨부 URL이 없다.
